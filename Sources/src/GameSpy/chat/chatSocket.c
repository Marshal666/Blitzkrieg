...
#if !defined(__KATANA__) && !defined(UNDER_CE)
#include <time.h>
#endif
...
extern void __cdecl CallAssert( int bCondition );
...
{
#if defined(__KATANA__) || defined(UNDER_CE) || defined(__mips64)
	return "";
#else
...
void ciSocketDisconnect(ciSocket * sock)
{
	...
	//////////////////////////////////
	gsifree(sock->lastMessage.message); sock->lastMessage.message = 0;
	gsifree(sock->lastMessage.server); sock->lastMessage.server = 0;
	gsifree(sock->lastMessage.nick); sock->lastMessage.nick = 0;
	gsifree(sock->lastMessage.user); sock->lastMessage.user = 0;
	gsifree(sock->lastMessage.host); sock->lastMessage.host = 0;
	gsifree(sock->lastMessage.command); sock->lastMessage.command = 0;
	gsifree(sock->lastMessage.middle); sock->lastMessage.middle = 0;
	gsifree(sock->lastMessage.param); sock->lastMessage.param = 0;
	for(i = 0 ; i < sock->lastMessage.numParams ; i++)
		gsifree(sock->lastMessage.params[i]);

	gsifree(sock->lastMessage.params);
	sock->lastMessage.params = 0;
}
