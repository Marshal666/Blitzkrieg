...
//	EventRecord rec;
	...
	#ifdef SN_SYSTEMS
		sn_delay(msec);
	#else //todo - need to do something else for other stacks
		;
	#endif 
#endif
#ifdef UNDER_UNIX
...
#else
#ifdef CISCO_NFT
	rcode = nftioctl(sock, UNTESTED_FIONBIO, (char*)&argp);
#else
...
#ifdef __mips64
static sceSifClientData  cd_sceILsockInit;
static sceSifClientData  cd_sce1394SbEui64;
static int isInit = 0;
...
enum SceILsockErrorCode	sceILsockInit(int maxsock, int maxsize)
{
  ...
  while( 1 )
  {
    ret = sceSifBindRpc( &cd_sceILsockInit, SIFNUM_sceILsockInit, 0 );
    if( cd_sceILsockInit.serve != 0 ) break;
    i = 0x10000;
    while( i-- );
  }
  ...
  while( 1 )
  {
    ret = sceSifBindRpc( &cd_sce1394SbEui64, SIFNUM_sce1394SbEui64, 0 );
    if( cd_sce1394SbEui64.serve != 0 ) break;
    i = 0x10000;
    while( i-- );
  }
  ...
  BufInit[0] = maxsock;
  BufInit[1] = maxsize;
  ret = sceSifCallRpc( &cd_sceILsockInit, SIFNUM_sceILsockInit, 0,
    (void*)BufInit, 16, (void*)BufInit, 16, 0, 0 );

  if( ret < 0 )
  {
    printf( "sceILsockInit(): rpc error\n" );
    return ret;
  }

  isInit = 1;

  printf( "sceILsockInit( maxsock=%d, maxsize=%d ) = %d\n", maxsock, maxsize, BufInit[0] );
...
enum SceILsockErrorCode sce1394SbEui64( unsigned int *buf )
{
  ...
  ret = sceSifCallRpc( &cd_sce1394SbEui64, SIFNUM_sce1394SbEui64, 0,
...
const char * GOAGetUniqueID(void)
{
	...
	ret = sceILsockInit( 3, SCEILSOCK_MAX_PAYLOAD_SIZE );
	...
	uid[0]='\0';
	sprintf(uid, "%08x%08x", MyGUID[0], MyGUID[1]);
	return uid;
}
...
const char * GOAGetUniqueID(void)
{
	...
	if (RegOpenKeyEx(HKEY_CURRENT_USER, REG_KEY, 0, KEY_ALL_ACCESS, &thekey) != ERROR_SUCCESS)
		...
		docreate = 0;
	ret = RegQueryValueEx(thekey, (LPCSTR)"Crypt", 0, &thetype, (LPBYTE)keyval, &len);
#else
	FILE *f;
	f = fopen("id.bin","r");
	if (!f)
