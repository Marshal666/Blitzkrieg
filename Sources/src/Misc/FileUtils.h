...
// ‘унктор дл€ EnumerateFiles, перечисл€ет все файлы в директории и берЄт их относительное им€
...
// ‘унктор дл€ EnumerateFiles, перечисл€ет все поддиректории в директории и берЄт их относительное им€
	...
	void operator() ( const NFile::CFileIterator &it )
	{
		if ( !it.IsDirectory() )
		{
			...
			if ( nRes == std::string::npos )
				return;
			szFileName = szFileName.substr( 0, nRes );	//обрезаем им€ файла, оставл€€ только директорию
