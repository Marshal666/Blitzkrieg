...
public:
	...
	// в 1 - некоторое малое врем€, чтобы произошло обновление, не 0 - т.к. это говорит о первом запуске сегмента
...
class CSoldierAttackState : public IUnitAttackingState, public CStandartBehaviour
{
	...
	// атака в лоб - ехать на юнит и, когда можно стрел€ть, стрел€ть
	void AnalyzeBruteMovingPosition();
	// идти на юнит до какого-то до него рассто€ни€, а потом зайти со стороны
...
class CSoldierEnterTransportNowState : public IUnitState
{
	...
	// последний апдейт траектории (нужно дл€ движущегос€ транспорта)
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// дл€ атаки самолетов. в этом стейте юнит следит не приближаютс€ ли
// к нему наземные цели. если кака€-то цель наземна€ по€вилась в радиусе видимости,
...
class CSoldierAttackAviationState : public IUnitAttackingState
{
	...
	enum ESoldierAttackAviationState
	{
		...
		SAAS_START_TRASING,										// дл€ стрельбы сопроводительным огнем
		SAAS_TRASING,
		SAAS_FIRING,
		
		SAAS_START_AIMING_TO_PREDICTED_POINT,	// дл€ стрельбы заградительным огнем
	...
	SPredict aimPoint;		// точка прицеливани€ при стрельбе заградительным огнем
