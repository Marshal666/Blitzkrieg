...
void CDirectoryPropsItem::UpdateItemValue( int nItemId, const CVariant &value )
{
	...
	if ( nItemId == 1 )
	{
		//»зменилось значение директории, загружаем все картинки из этой диры в AllThumbList
		//ѕровер€ем, что этот TreeItem выделен в дереве, иначе делаю его выделенным
...
bool CAnimationTreeRootItem::ComposeAnimations( const char *pszProjectFileName, const char *pszResultingDir, bool bSetCycledFlag, bool bShowUserMessages )
{
	...
	//узнаем, есть директори€ blood в директории проекта, или нет
...
FILETIME CAnimationTreeRootItem::FindMaximalSourceTime( const char *pszProjectFileName )
{
	...
	for ( CTreeItemList::const_iterator seasonIt=pDirsItem->GetBegin(); seasonIt!=pDirsItem->GetEnd(); ++seasonIt )
	{
		...
		for ( animIt=pAnimsItem->GetBegin(); animIt!=pAnimsItem->GetEnd(); ++animIt )
		{
			...
			//«десь надо будет сделать дополнительное прохождение по сезонам...
	...
	//ѕроходим по всем файлам и находим максимальное врем€ изменени€
	FILETIME nMaxTime;
...
bool CUnitAnimationPropsItem::CopyItemTo( CTreeItem *pToItem )
{
	...
	//“еперь копируем список CUnitFramePropsItem
	pTo->RemoveAllChilds();
...
/*
	CSaverAccessor saver = pSS;
	//—охран€ем имена внутри m_selectedItems
...
{
	//¬ ThumbList отображаю Animations соответствующие этой папке
	CTreeItem *pPapa = GetParentTreeItem();
	...
	//¬ накиданных ThumbList items выдел€ю item соответствующий this
...
void CUnitFramePropsItem::MyKeyDown( int nChar )
{
	switch ( nChar )
	{
		case VK_DELETE:
		/*
		//—мотрим какой frame будет следующим выделенным в дереве и выдел€ем его в SelectedThumbList
