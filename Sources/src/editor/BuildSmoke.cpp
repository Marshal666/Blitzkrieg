...
void CBuildingFrame::SelectSmokePoint( CBuildingSmokePropsItem *pSmokePoint )
{
...
void CBuildingFrame::DeleteSmokePoint()
{
	if ( !pActiveSmokePoint )
		return;
...
void CBuildingFrame::ComputeSmokeLines()
{
...
void CBuildingFrame::SetHorSmokePoint( const POINT &point )
{
...
void CBuildingFrame::SetSmokePointAngle( const POINT &point )
{
...
void CBuildingFrame::MoveSmokePoint( const POINT &point )
{
	if ( !pActiveSmokePoint )
		return;
...
void CBuildingFrame::AddOrSelectSmokePoint( const POINT &point )
{
	CVec2 smokePos2;
	smokePos2.x = point.x;
	smokePos2.y = point.y;
...
void CBuildingFrame::GenerateSmokePoints()
{
	IScene *pSG = GetSingleton<IScene>();
	IGameTimer *pTimer = GetSingleton<IGameTimer>();
	...
	for ( int i=0; i<(nTileMaxX-nTileMinX+1)/2; i++ )
	{
		...
		CBuildingSmokePropsItem *pProps = new CBuildingSmokePropsItem;
...
void CBuildingFrame::CreateSmokeSprites( CBuildingSmokePropsItem *pSmokePoint )
{
	IVisObjBuilder *pVOB = GetSingleton<IVisObjBuilder>();
	IScene *pSG = GetSingleton<IScene>();
	...
	pSmokePoint->pSprite->SetPosition( m_SpriteLoadPos );
	...
	pSmokePoint->pHLine->SetPosition( m_SpriteLoadPos );
